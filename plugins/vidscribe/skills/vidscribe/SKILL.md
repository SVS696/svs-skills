...
version: 0.2.0
description: |
  Локальная транскрипция видео и встреч через CLI `vidscribe`: ffmpeg извлекает
  аудио и кадры, faster-whisper + pyannote дают спикер-aware чанки, CLI-провайдеры
  (claude / codex / ollama) правят текст и добавляют визуальный контекст, ассемблер
  собирает итоговый markdown.
  Use when the user asks to: распознай / транскрибируй / расшифруй
  видео|встречу|запись|интервью|лекцию|скринкаст; transcribe a video, meeting,
  screen recording, Zoom/Teams/Telemost recording; describe on-screen action +
  speech; regenerate or fix an existing transcript (другая модель, имена спикеров).
license: MIT
---
...
> **Установка CLI.** Этот скилл только вызывает CLI `vidscribe`, он не входит в
> плагин. Поставь его отдельно: `git clone https://github.com/SVS696/vidscribe`
> и `pip install -e .` в репозитории (нужны ffmpeg, faster-whisper, pyannote).
> Проверка: `vidscribe --help`.
